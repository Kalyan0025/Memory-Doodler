...
attendees = max(3, min(48, attendees))  # clamp final
...
new p5((p)=>{{
  ...
  function drawReunion(){{
    ...
    let labels = "{'|'.join(schema['labels'])}".split("|").filter(Boolean); let labelIdx=0;
    for (let g=0; g<centers.length; g++){{ const gx=centers[g][0], gy=centers[g][1];
      ...
      for (let i=0;i<n;i++){{ const a=i/Math.max(1,n) * p.TWO_PI - p.HALF_PI + (g*0.3); const jx=gx + r*Math.cos(a); const jy=gy + r*Math.sin(a);
        ...
        if ({str(show_names).lower()} && labels.length>0 && labelIdx<labels.length){{ p.fill(90,70,60,150); p.textAlign(p.CENTER,p.TOP); p.textSize(10); p.text(labels[labelIdx], jx, jy+14); labelIdx++; }}
      }}
    }}
  }}
  function drawLegend(){{
    if (!{str(show_legend).lower()}) return;
